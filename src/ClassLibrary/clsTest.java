...
public class clsTest {
    ...
    static void PrintTrip(clsSolution solution){
        System.out.println("Trip Itinerary for period from "+clsData.getStartPoint()[5]+" until "+clsData.getStartPoint()[6]+".");
        System.out.println("-----------");
        for(int i=0;i<solution.Itinerary.length;i++){
            ...
            for(int j=0;j<=solution.TourLastVisit[1][i];j++){
             ...
             System.out.println("---");
             System.out.println("chechkTimeSpent="+chechkTripTimeSpent(solution));
             System.out.println("Time spent of tour "+i+" "+solution.TourTimeSpent[i]);
             System.out.println("Left time:"+(clsData.getStartPoint()[6]-solution.TourTimeSpent[i]));
        ...
        static void StartTimeTest(clsSolution Solution)
    {
        ...
        static void OnlyStartTimeTest(clsSolution Solution){//
        ...
        for(int i=0;i<Solution.Itinerary.length;i++)
        {
            for(int j=0;j<Solution.TourLastVisit[1][i];j++)
            {
                //Test
                if(j==0){
                    StartTime=clsGeneral.getDistanceBetweenPoints(1, Solution.Itinerary[i][j].POI_ID, clsData.getDistance());
                }
                else{
                    StartTime=Solution.Itinerary[i][j-1].constraints.TimeWindow.EndTime +
                            ...
                            Solution.Itinerary[i][j].POI_ID, clsData.getDistance());
                }
        ...
        static void OnlyStartTimeTest(clsSolution CurrentSolution,clsOperatorSolution SwapSolution){ 
            float StartTime;
            if(SwapSolution.VisitIndex==0){
                    StartTime=clsGeneral.getDistanceBetweenPoints(1, 
                           SwapSolution.InsertedPOI_ID, clsData.getDistance());
                }
                else{
        ...
        static boolean chechkCost(clsSolution Solution){//Initial Solution, Swap OK
            boolean rez=true;
            float RealCost=0;
            for(int i=0;i<Solution.Itinerary.length;i++)
            {
                for(int j=0;j<=Solution.TourLastVisit[1][i];j++)
                {
                    RealCost=RealCost+ clsData.getPOI()[Solution.Itinerary[i][j].POI_ID-2][10+clsData.getNumberOfPersons()] ;
                }
            }
        ...
        static boolean chechkTimeWindows(clsSolution Solution)
        {
            boolean rez=true;
            for(int i=0;i<Solution.Itinerary.length;i++)
            {
        ...
        static boolean chechkTypeConstraintCounting(clsSolution Solution){
            boolean result=true;
            ...
            for(int i=0;i<Solution.Itinerary.length;i++)
            {
                ...
                {
                    for(int k=0;k<10;k++){
                        RealTypeCounter[k]+=clsData.getPOI()[Solution.Itinerary[i][j].POI_ID-2][11+clsData.getNumberOfPersons()+k];
                    }
                }
            }            
        ...
        static boolean chechkTypeConstraintLimit(clsSolution Solution){
            boolean result=true;
            int MaximumNumberOfVerticesOfTypeZ[]=clsData.getMaximumNumberOfVerticesOfTypeZ();
            for(int i=0;i<10;i++){
        ...
        static boolean chechkTripTimeSpent(clsSolution Solution){
            boolean result=true;
            ...
            for(int i=0;i<Solution.Itinerary.length && Solution.Itinerary[i][0]!=null ;i++){
                RealTourSpentTime[i]=Solution.Itinerary[i][0].constraints.TimeWindow.UnusedTimeAtTheStartOfTheTour+
                        clsGeneral.getDistanceBetweenPoints(1, Solution.Itinerary[i][0].POI_ID, clsData.getDistance());
            }
            ...
            for(int i=0;i<Solution.Itinerary.length && Solution.Itinerary[i][0]!=null;i++)
            {
                for(int j=0;j<=Solution.TourLastVisit[1][i];j++)
                {
                    if(j!=Solution.TourLastVisit[1][i]){
                        RealTourSpentTime[i]+=(clsData.getPOI()[Solution.Itinerary[i][j].POI_ID-2][3]+
                                clsGeneral.getDistanceBetweenPoints(Solution.Itinerary[i][j].POI_ID, 
                                Solution.Itinerary[i][j+1].POI_ID, clsData.getDistance()));
                    }
                    else{
                        RealTourSpentTime[i]+=(clsData.getPOI()[Solution.Itinerary[i][j].POI_ID-2][3]+
                                clsGeneral.getDistanceBetweenPoints(Solution.Itinerary[i][j].POI_ID, 
...
static boolean chechkTourTimeSpent(clsVisit Itinerary[], int TourLastVisit, float TourTimeSpent){
            boolean result=true;
            float RealTourSpentTime;
                RealTourSpentTime=clsGeneral.getDistanceBetweenPoints(1, Itinerary[0].POI_ID, clsData.getDistance());
           
                for(int j=0;j<=TourLastVisit;j++)
                {
                    if(j!=TourLastVisit){
                        RealTourSpentTime+=(clsData.getPOI()[Itinerary[j].POI_ID-2][3]+
                                clsGeneral.getDistanceBetweenPoints(Itinerary[j].POI_ID, 
                                Itinerary[j+1].POI_ID, clsData.getDistance()));
                    }
                    else{
                        RealTourSpentTime+=(clsData.getPOI()[Itinerary[j].POI_ID-2][3]+
                                clsGeneral.getDistanceBetweenPoints(Itinerary[j].POI_ID, 
        ...
        static boolean chechkEvaluation(clsSolution Solution){
            boolean result=true;
            ...
            for(int i=0;i<Solution.Itinerary.length;i++)
            {
                for(int j=0;j<=Solution.TourLastVisit[1][i];j++)
                {
                    for(int k=0;k<clsData.getNumberOfPersons();k++){
                        RealEvaluation+=clsData.getPOI()[Solution.Itinerary[i][j].POI_ID-2][4+k];
                    }
                }
            }   
