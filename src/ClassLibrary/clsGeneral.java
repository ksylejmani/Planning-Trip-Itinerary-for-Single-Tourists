...
public static float round(float value, int places) {
    ...
    static clsTypeConstraint CheckTypeConstraint(int TypeCounter[], int TypeLimit[],
        ...
        else
        {
         OuterFor:for(int i=TourIndexToStart;i<TourSpentTime.length;i++){
            ...
            if(LastVisit[0][i]==0)
            {
                 ...
                 {
                     UnusedTimeAtTheStartOfTheTour=getTourUnusedTimeAtTheStart(POI_ID);
                 }
            }
            else
            {
...
//            System.out.println("Stop");
   ...
   static float getMaxRightShift(int POI_ID, float VisitEndTime) {
        float result;
        result = clsData.getPOI()[POI_ID - 2][8 + clsData.getNumberOfPersons()] - VisitEndTime;
        return result;
    }
    static float  getMaxLeftShift(int POI_ID, float VisitStartTime) {
        float result;
        result = VisitStartTime-clsData.getPOI()[POI_ID - 2][4 + clsData.getNumberOfPersons()] ;
        return result;
    }
...
//                //Test
 ...
 static int [] getMaxShiftNeighbourImpact(int TourIndex,int TourLastVisit,
    ...
    static float getTourUnusedTimeAtTheStart(int POI_ID){
        float result=0;
        float DistanceFromStartPointToCurrentPOI=getDistanceBetweenPoints(1,POI_ID,clsData.getDistance());
        float OpeningTimeOfPOI=clsData.getPOI()[POI_ID-2][clsData.getNumberOfPersons()+4];
        if(DistanceFromStartPointToCurrentPOI<OpeningTimeOfPOI)
            result=OpeningTimeOfPOI-DistanceFromStartPointToCurrentPOI-clsData.getStartPoint()[5];
        return result;
    }
