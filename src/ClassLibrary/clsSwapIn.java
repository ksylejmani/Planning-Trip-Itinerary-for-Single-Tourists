...
void createSwapSolution(int IterationCounter, int RecencyMemory[][], int TABU_LIST_SIZE) {
        clsOperatorSolution TabooSolution = new clsOperatorSolution();
        clsOperatorSolution NonTabooSolution = new clsOperatorSolution();
        TabooSolution.TimeCost=NonTabooSolution.TimeCost=this.getInitialSolutionTimeCost();
             ...
             for (int j = 0; j<=solution.TourLastVisit[1][i]; j++) {
                if((i==solution.Itinerary.length-1 ) && (j==solution.TourLastVisit[1][i]))
                 ...
                 for (int k = i; k <solution.Itinerary.length; k++) {
                    ...
                    if(j==0){
                        UnusedTimeAtTheStartOfRemovingTour=clsGeneral.getTourUnusedTimeAtTheStart(InsertedPOI_ID);
                    }
                    if(l==0){
                        UnusedTimeAtTheStartOfInsertingTour=clsGeneral.getTourUnusedTimeAtTheStart(RemovedPOI_ID);
                    }
                    float DistanceFromPreviosPointToInsertedPointInRemoveingTour=this.getDistanceFromPreviosPoint(i, j, InsertedPOI_ID);
                    float DistanceFromInsertedPointToNextPointInRemoveingTour=
                            this.getDistanceToNextPointWithPossibleSwapBetweenNeighbourPOIs(i, j, InsertedPOI_ID, 
                            solution.TourLastVisit[1][i],SwapBetweenNeighbourPOIs);
                    float RemoveingTourShiftingValue=this.getShiftingValue(RemovedPOI_ID, InsertedPOI_ID, 
                    ...
                    float DistanceFromPreviosPointToInsertedPointInInsertingTour=
                            this.getDistanceFromPreviosPointWithPossibleSwapBetweenNeighbourPOIs(k, l, RemovedPOI_ID,SwapBetweenNeighbourPOIs);
                    float DistanceFromInsertedPointToNextPointInInsertingTour=this.getDistanceToNextPoint(k, l, RemovedPOI_ID, solution.TourLastVisit[1][k]);
                    float InsertingTourShiftingValue=this.getShiftingValue(InsertedPOI_ID,RemovedPOI_ID, 
                    ...
                    float[] IndicatedStartAndEndTime =this.getIndicatedStartAndEndTime(i, j, k, l, 
...
float[] getIndicatedStartAndEndTime(int TourIndexInRemoveingTour, int VisitIndexInRemoveingTour, 
        ...
        float IndicatedEndTimeInRemoveingTour = IndicatedStartTimeInRemoveingTour + 
                clsData.getPOI()[solution.Itinerary[TourIndexInInsertingTour][VisitIndexInInsertingTour].POI_ID - 2][3];

        if ((IndicatedStartTimeInRemoveingTour >= 
                clsData.getPOI()[solution.Itinerary[TourIndexInInsertingTour][VisitIndexInInsertingTour].POI_ID - 2][4 + clsData.getNumberOfPersons()])
                && (IndicatedEndTimeInRemoveingTour <= 
                clsData.getPOI()[solution.Itinerary[TourIndexInInsertingTour][VisitIndexInInsertingTour].POI_ID - 2][8 + clsData.getNumberOfPersons()])) 
        {
            ...
            if (IndicatedTourSpentTimeInRemoveingTour <= clsData.getStartPoint()[6]) {
                ...
                if(TourIndexInRemoveingTour==TourIndexInInsertingTour){
                    ...
                    clsData.getPOI()[RemovedPOIID-2][3],clsData.getPOI()[InsertedPOIID-2][3]);
                }
                    if(TimeWindowFeasibilityOfFollowingPOIsInRemoveingTour){
                        ...
                        float IndicatedEndTimeInInsertingTour = IndicatedStartTimeInInsertingTour + 
                                clsData.getPOI()[solution.Itinerary[TourIndexInRemoveingTour][VisitIndexInRemoveingTour].POI_ID - 2][3];
                        if((IndicatedStartTimeInInsertingTour >= 
                            clsData.getPOI()[solution.Itinerary[TourIndexInRemoveingTour][VisitIndexInRemoveingTour].POI_ID - 2]
                            ...
                            if(IndicatedTourSpentTimeInInsertingTour <= clsData.getStartPoint()[6]){
                                ...
                                if(TourIndexInRemoveingTour==TourIndexInInsertingTour){
                                 ...
                                 else
                                 {
                                    ...
                                    clsData.getPOI()[InsertedPOIID-2][3],clsData.getPOI()[RemovedPOIID-2][3]); 
                                 }   
...
//        System.out.println("Test");
    ...
    float getShiftingValue(int RemovedPOI_ID, int InsertedPOI_ID, 
    ...
    float getDistanceFromPreviosPoint(int POITourIndeks,int POIVisitIndeks,
        int POI_ID){
        float result;
        if (POIVisitIndeks == 0) {
            result =
    ...
    float getDistanceFromPreviosPointWithPossibleSwapBetweenNeighbourPOIs(int POITourIndeks,int POIVisitIndeks,
    ...
    float getDistanceToNextPoint(int POITourIndeks,int POIVisitIndeks,int POI_ID,
    ...
    float getDistanceToNextPointWithPossibleSwapBetweenNeighbourPOIs(int POITourIndeks,int POIVisitIndeks,int POI_ID,
    ...
    float getInitialSolutionTimeCost(){
        float result;
            result=clsData.getStartPoint()[6]*clsData.getNumberOfTours();
        return result;
    }
