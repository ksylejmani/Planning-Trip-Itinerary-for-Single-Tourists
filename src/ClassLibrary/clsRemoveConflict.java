...
public class clsRemoveConflict {
    ...
    clsSolution getSolutionWithoutTWConflicts(clsSolution Solution){
       ...
       for(int i=0;i<ListToRepair.size();i++){
          ...
          applySingleRemoveWithConflict(Solution,TourIndex,VisitIndex,POIWithTimeWindowConflict);
          ListToRepair= getListOfPOIsOutOfTimeWindows(Solution);
    ...
    clsRemoveSolution applySingleRemoveConflict(clsSolution Solution, int TourIndex, int VisitIndex, int POIWithTimeWindowConflict)
    {
        clsRemoveSolution result=new clsRemoveSolution();
        float ShiftingValue=0;
        int TourLastVisit=Solution.TourLastVisit[1][TourIndex];
        ShiftingValue=this.getShiftingValue(Solution,TourIndex, VisitIndex,TourLastVisit );
        ...
        result.DistanceFromPreviousPointToNextPoint=this.getDistanceFromPreviousPointToNextPoint(Solution,result.TourIndex, result.VisitIndex);
        return result;
    }
void applySingleRemoveWithConflict(clsSolution CurrentSolution, int TourIndex, int VisitIndex, int POIWithTimeWindowConflict){
            clsRemoveSolution RemoveSolution= applySingleRemoveConflict(CurrentSolution,TourIndex,VisitIndex,POIWithTimeWindowConflict);
...
//            //Update Start time and End time of Preceiding POIs
    ...
    float getDistanceFromPreviousPointToNextPoint(clsSolution Solution,int TourIndex, int VisitIndex){
        float result;
        if(VisitIndex==0){           
            result=clsGeneral.getDistanceBetweenPoints(1, Solution.Itinerary[TourIndex][VisitIndex+1].POI_ID, clsData.getDistance());
        }           
        else if(VisitIndex==Solution.TourLastVisit[1][TourIndex])
            result=clsGeneral.getDistanceBetweenPoints(Solution.Itinerary[TourIndex][VisitIndex-1].POI_ID,1 , clsData.getDistance());
        else
            result=clsGeneral.getDistanceBetweenPoints(Solution.Itinerary[TourIndex][VisitIndex-1].POI_ID,
                    Solution.Itinerary[TourIndex][VisitIndex+1].POI_ID , clsData.getDistance());
        return result;
    }
...
//    }
    ...
    float getShiftingValue(clsSolution Solution,int TourIndex, int VisitIndex,int TourLastVisit){//Te shiqohet efekti ne te gjitha vendet ku perdoret getShiftingValue
            float result;
            if(VisitIndex==TourLastVisit){
                ...
                result=Solution.Itinerary[TourIndex][VisitIndex].constraints.TimeWindow.DistanceFromPrevioiusPoint+
                        ...
                        clsGeneral.getDistanceBetweenPoints(Solution.Itinerary[TourIndex][VisitIndex-1].POI_ID,1,  clsData.getDistance());                
            }
                ...
                result=Solution.Itinerary[TourIndex][VisitIndex].constraints.TimeWindow.DistanceFromPrevioiusPoint+
                        ...
                        Solution.Itinerary[TourIndex][VisitIndex].constraints.TimeWindow.StartTime)+
                        Solution.Itinerary[TourIndex][VisitIndex].constraints.TimeWindow.DistanceToNextPoint-
                        clsGeneral.getDistanceBetweenPoints(1, Solution.Itinerary[TourIndex][VisitIndex+1].POI_ID, clsData.getDistance());
            }
            else
            {
                result= 
    ...
    static void updateCost(clsSolution CurrentSolution, clsRemoveSolution RemoveSolution){
        float RemovedPOICost=clsData.getPOI()[RemoveSolution.RemovedPOI_ID-2][10+clsData.getNumberOfPersons()];
        CurrentSolution.BudgetCost-=RemovedPOICost;
    }
    static void updateEvaluation(clsSolution CurrentSolution, int RemovedPOI_ID) {
        for (int i = 0; i < clsData.getNumberOfPersons(); i++) {
            CurrentSolution.Evaluation -= clsData.getPOI()[RemovedPOI_ID - 2][4 + i];
        }
    }
    ...
    static void updateTypeConstraint(int TypeCounter[],float POI[]){
        for(int i=0;i<10;i++){
            if(POI[i+11+clsData.getNumberOfPersons()]==1)
                TypeCounter[i]--;
        }
    }
static void updateTourMaxShift(int TourIndex, int TourLastVisit[][], 
        clsVisit Itinerary[][], float MaxShift[][][]) {
            ...
            MaxShift[TourIndex][0][TourLastVisit[1][TourIndex]]=
                    clsGeneral.getMaxRightShift(Itinerary[TourIndex][TourLastVisit[1][TourIndex]].POI_ID,
            ...
            MaxShift[TourIndex][1][TourLastVisit[1][TourIndex]]=
                    clsGeneral.getMaxLeftShift(Itinerary[TourIndex][TourLastVisit[1][TourIndex]].POI_ID,
            ...
            for(int i=TourLastVisit[1][TourIndex]-1;i>=0;i--){
                 ...
                 CurrentVisitMaxRightShift= clsGeneral.getMaxRightShift(Itinerary[TourIndex][i].POI_ID,
                            Itinerary[TourIndex][i].constraints.TimeWindow.EndTime);
                 CurrentVisitMaxLeftShift= clsGeneral.getMaxLeftShift(Itinerary[TourIndex][i].POI_ID,
