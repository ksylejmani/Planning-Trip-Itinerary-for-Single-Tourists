...
public  clsSolution getTwoOptSolution(clsSolution solution){
    ...
    for(int TourIndex=0;TourIndex<solution.Itinerary.length;TourIndex++){
        ...
        clsTwoOptTourSolution TwoOptTourSolution=this.getTwoOptTourSolution(
...
float getSolutionTimeCost(float TourTimeSpent[]){
 ...
 clsTwoOptTourSolution getTwoOptTourSolution(clsVisit TourItinerary [],int TourIndex,int TourLastVisitIndex,
         float TourSpentTime,
         float TourMaxShift[][]){
     ...
     { 
          ArrayList<float []> TourDoubleEdges=this.extractTourDoubleEdges(
                ...
                //Test
                clsTest.chechkTourTimeSpent(CurrentTourItinerary, TourLastVisitIndex, CurrentTourSpentTime);
                //Test
          clsTwoOptTourSolution CurrentTwoOptSolution=this.applyTwoOpt(
          ...
          if(CurrentTwoOptSolution.LeftPOI_ID!=-1 && CurrentTwoOptSolution.TourTimeCost<CurrentTourSpentTime){
              result.ThereIsImprovement=true;
                ...
                clsTest.chechkTourTimeSpent(CurrentTourItinerary, TourLastVisitIndex, CurrentTourSpentTime);
                //Test
              this.updateTourItinerary(CurrentTourItinerary,TourLastVisitIndex,CurrentTwoOptSolution);            
              CurrentTourSpentTime= CurrentTwoOptSolution.TourTimeCost;
              
            //Test
            clsTest.chechkTourTimeSpent(CurrentTourItinerary, TourLastVisitIndex, CurrentTourSpentTime);
            //Test
              this.updateTourMaxShift(TourLastVisitIndex, CurrentTourItinerary, CurrentTourMaxShift);
 ...
 void updateTourItinerary(clsVisit CurrentTourItinerary[],int TourLastVisitIndex, 
         clsTwoOptTourSolution CurrentTwoOptSolution){
    this.updateAffectedPOIsAfterTwo_Opt(CurrentTourItinerary,TourLastVisitIndex, CurrentTwoOptSolution);
 ...
 clsTwoOptTourSolution applyTwoOpt(clsVisit TourItinerary [],int TourIndex,int TourLastVisit,float TourSpentTime,
    ...
    for(int i=0;i<TourDoubleEdges.size()/4;i++){// one fourth of duble edges to try
        ...
        result=twooptswap.createTwoOptSolution(TourIndex,LeftPOIIndex, RightPOIIndex, LeftPOIID, RightPOIID, 
...
float getTourSpentTime(float TourSpentTime, int TourLastVisit, clsVisit TourItinerary[], int LeftPOIID, int RightPOIID,
        int LeftPOIIndex, int RightPOIIndex){
    float result;
    float TourShiftingValue=this.getTourShiftingValue(TourSpentTime, TourItinerary, LeftPOIID, RightPOIID, 
                LeftPOIIndex, RightPOIIndex, TourLastVisit);
    result=TourSpentTime-TourShiftingValue;
    return result;
} 
float getTourDurationChangeBeforeLeftOpt(int OptPOI_ID, int LeftOptIndex,clsVisit TourItinerary[]){
    float result;
    if(LeftOptIndex==0)
        result=clsGeneral.getDistanceBetweenPoints(1, OptPOI_ID, clsData.getDistance());
    else
        result=clsGeneral.getDistanceBetweenPoints(TourItinerary[LeftOptIndex-1].POI_ID, OptPOI_ID, clsData.getDistance());
    return result;
}
float getTourDurationChangeAfterLeftOpt(int OptPOI_ID, int LeftOptIndex,clsVisit TourItinerary[]){
    float result;
        result=clsGeneral.getDistanceBetweenPoints(OptPOI_ID,TourItinerary[LeftOptIndex+1].POI_ID, clsData.getDistance());
    return result;
}
float getTourDurationChangeAfterRightOpt(int OptPOI_ID, int RightOptIndex,clsVisit TourItinerary[], int TourLastVisit){
    float result;
    if(RightOptIndex==TourLastVisit)
        result=clsGeneral.getDistanceBetweenPoints(OptPOI_ID,1 , clsData.getDistance());
    else
        result=clsGeneral.getDistanceBetweenPoints(OptPOI_ID,TourItinerary[RightOptIndex+1].POI_ID , clsData.getDistance());
    return result;
}
float getTourDurationChangeBeforeRightOpt(int OptPOI_ID, int RightOptIndex,clsVisit TourItinerary[], int TourLastVisit){
    float result;
        result=clsGeneral.getDistanceBetweenPoints(TourItinerary[RightOptIndex-1].POI_ID, OptPOI_ID , clsData.getDistance());
    return result;
}
int [] findEdgesToRemove(clsVisit TourItinerary[],int TourLastVisit){
 ...
 ArrayList<float []> extractTourDoubleEdges(clsVisit TourItinerary[],int TourLastVisit){
    ...
    float getTourShiftingValue(float TourSpentTime,clsVisit TourItinerary[], int LeftOptID,int RightOptID,
        ...
        float AddedLength=this.getTourDurationChangeBeforeLeftOpt(RightOptID, LeftPOIIndex, TourItinerary)+
                this.getTourDurationChangeAfterLeftOpt(RightOptID, LeftPOIIndex, TourItinerary)+
                this.getTourDurationChangeBeforeRightOpt(LeftOptID, RightPOIIndex, TourItinerary, TourLastVisit)+
                this.getTourDurationChangeAfterRightOpt(LeftOptID, RightPOIIndex, TourItinerary, TourLastVisit);
    ...
    void updateAffectedPOIsAfterTwo_Opt(clsVisit CurrentTourItinerary[], int TourLastVisitIndex,clsTwoOptTourSolution CurrentTwoOptSolution){
           ...
           if(CurrentTwoOptSolution.RightPOIIndex==CurrentTwoOptSolution.LeftPOIIndex+1){
            ...
            else
            {
                ...
                this.updateTourInRemoveingPart(CurrentTourItinerary, CurrentTwoOptSolution.LeftPOIIndex,
                ...
                this.updateTourInInsertingPart(CurrentTourItinerary,TourLastVisitIndex, CurrentTwoOptSolution.RightPOIIndex, 
                    CurrentTwoOptSolution.RightVisitEndTime, CurrentTwoOptSolution.LeftPOI_ID, 
                    CurrentTwoOptSolution.RightDistanceFromPrevioiusPoint, 
                    CurrentTwoOptSolution.RightDistanceToNextPoint);
            }
...
void updateTourInRemoveingPart(clsVisit CurrentTourItinerary[],int VisitIndex,int InsertingTourIndex,
        ...
        boolean IsSubTourReversed,clsVisit SubTourItinerary[], float DistanceFromPreviousPointInInsertingTour){
           if(IsSubTourReversed){
              ...
              for(int i=VisitIndex+1;i<InsertingTourIndex;i++){           
                ...
                VisitTime+=clsData.getPOI()[CurrentTourItinerary[i].POI_ID-2][3];
    ...
    void updateTourInInsertingPart(clsVisit CurrentTourItinerary[],int TourLastVisitIndex,int VisitIndex,float VisitEndTime,int InsertedPOI_ID,
...
void updateTour(clsVisit CurrentTourItinerary[],int TourLastVisitIndex,int VisitIndex,float VisitEndTime,int InsertedPOI_ID,
            float DistanceFromPrevioiusPoint, float DistanceToNextPoint){
        //Update Start time and End time of Preceiding POIs
 ...
 void updateTourMaxShift(int TourLastVisitIndex, clsVisit Itinerary[], float MaxShift[][]) {
            MaxShift[0][TourLastVisitIndex]=
                    clsGeneral.getMaxRightShift(Itinerary[TourLastVisitIndex].POI_ID,
                    Itinerary[TourLastVisitIndex].constraints.TimeWindow.EndTime);
            MaxShift[1][TourLastVisitIndex]=
                    clsGeneral.getMaxLeftShift(Itinerary[TourLastVisitIndex].POI_ID,
            ...
            for(int i=TourLastVisitIndex-1;i>=0;i--){
                 ...
                 CurrentVisitMaxRightShift= clsGeneral.getMaxRightShift(Itinerary[i].POI_ID,
                            Itinerary[i].constraints.TimeWindow.EndTime);
                 CurrentVisitMaxLeftShift= clsGeneral.getMaxLeftShift(Itinerary[i].POI_ID,
