        ...
        for (int i = 0; i < solution.Itinerary.length &&  solution.Itinerary[i]!=null; i++) {
            for (int j = 0; j <= solution.TourLastVisit[1][i] && solution.TourLastVisit[0][i]!=0; j++) {
                for (int k = 0; k < solution.POIsOffItinerary.size(); k++) {
                    ...
                    if(j==0){
                        TourUnusedTimeAtTheStart=clsGeneral.getTourUnusedTimeAtTheStart(InsertedPOI_ID);
                    ...
                    float Cost = this.getBudgetFeasibility(clsData.getBudgetLimitation(), solution.BudgetCost,
                        ...
                        if (TypeConstraintFeasibility.Reply == 1) {
                            ...
                            if (IndicatedStartAndEndTime[0] >= 0) {
                                float CurrentSolutionEvaluation =
                                        this.getEvaluation(RemovedPOI_ID, InsertedPOI_ID,
                                        solution.Evaluation);
...
//                                        clsData.getPOI()[RemovedPOI_ID-2][3];
    ...
    float[] getIndicatedStartAndEndTime(int POITourIndeks, int POIVisitIndeks, int IndeksOfPOIinOffList, int IndeksOfLastVisitIntour,
        ...
        if ((IndicatedStartTime >= clsData.getPOI()[solution.POIsOffItinerary.get(IndeksOfPOIinOffList) - 2][4 + clsData.getNumberOfPersons()])
            ...
            float IndicatedTourSpentTime = getIndicatedTourSpentTime(POITourIndeks, POIVisitIndeks, IndeksOfPOIinOffList,
                    DistanceFromPreviosPointToSwappingPoint,DifferenceOffUnusedTourTimeAtTheStart);
            if (IndicatedTourSpentTime <= clsData.getStartPoint()[6]) {//Testimi OK
                boolean TimeWindowFeasibilityOfFollowingPOIs;
                    TimeWindowFeasibilityOfFollowingPOIs =//Te kontrollohet....
                    getTimeWindowFeasibilityOfAffectedPOIs(POITourIndeks, POIVisitIndeks,
...
//                    
    ...
    float getIndicatedTourSpentTime(int POITourIndeks, int POIVisitIndeks, int IndeksOfPOIinOffList, 
            float DistanceFromPreviosPointToSwappingPoint,float DifferenceOffUnusedTourTimeAtTheStart) {
        float result;
        float DistanceFromSwappingPointToNextPoint;
    ...
    clsTypeConstraint getTypeConstraintFeasibility(int TypeCounter[], int TypeLimit[],
    ...
    private float getEvaluation(int RemovedPOI_ID,
...
//            }
 ...
 boolean getTimeWindowFeasibilityOfAffectedPOIs(int TourIndex,int VisitIndex,int TourLastVisit, int IndexOfPOIInOffList,
