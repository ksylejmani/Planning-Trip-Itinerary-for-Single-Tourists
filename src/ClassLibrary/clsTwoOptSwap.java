...
clsTwoOptTourSolution createTwoOptSolution(int TourIndex, int LeftPOIIndex,int RightPOIIndex,int LeftPOIID,int RightPOIID,
 int TourLastVisit,float TourSpentTime,clsVisit TourItinerary[], float TourMaxShift[][]) {
    ...
    clsTwoOptTourSolution result = new clsTwoOptTourSolution();
        result.TourTimeCost=this.getInitialSolutionTimeCost();
        ...
        if(SubTourGenerationNeeded){
            ...
            float NonReversedTwoOptChangeLength=this.getTwoOptChangeLength(NonReversedSubTourItinerary, LeftPOIID, RightPOIID);
            ...
            float ReversedSubTourDuration=this.getSubtourLength(ReversedSubTourItinerary, 
                    TourItinerary[LeftPOIIndex].POI_ID,  TourItinerary[RightPOIIndex].POI_ID);
            float ReversedTwoOptChangeLength=this.getTwoOptChangeLength(ReversedSubTourItinerary, LeftPOIID, RightPOIID);
        ...
        float DistanceFromPreviosPointToInsertedPointInRemoveingTour=
                this.getDistanceFromPreviosPoint(LeftPOIIndex, RightPOIID,TourItinerary);
        float DistanceFromInsertedPointToNextPointInRemoveingTour=
                this.getDistanceToNextPointWithPossibleSwapBetweenNeighbourPOIs(LeftPOIIndex,RightPOIIndex, RightPOIID, 
                SwapBetweenNeighbourPOIs,TourItinerary,IsSubTourReversed);       
        
        float RemoveingTourShiftingValue=this.getShiftingValue(LeftPOIID, RightPOIID, 
        ...
        float DistanceFromPreviosPointToInsertedPointInInsertingTour=
                this.getDistanceFromPreviosPointWithPossibleSwapBetweenNeighbourPOIs(
                LeftPOIIndex,RightPOIIndex, LeftPOIID,SwapBetweenNeighbourPOIs,TourItinerary,IsSubTourReversed);
        float DistanceFromInsertedPointToNextPointInInsertingTour=this.getDistanceToNextPoint(RightPOIIndex, LeftPOIID, 
                TourLastVisit,TourItinerary);
        float InsertingTourShiftingValue=this.getShiftingValue(RightPOIID,LeftPOIID, 
        ...
        float[] IndicatedStartAndEndTime =this.getIndicatedStartAndEndTime( 
...
float[] getIndicatedStartAndEndTime( int LeftVisitIndex, 
        ...
        float IndicatedEndTimeInRemoveingTour = IndicatedStartTimeInRemoveingTour + 
                clsData.getPOI()[TourItinerary[RightVisitIndex].POI_ID - 2][3];

        if ((IndicatedStartTimeInRemoveingTour >= 
                clsData.getPOI()[TourItinerary[RightVisitIndex].POI_ID - 2][4 + clsData.getNumberOfPersons()])
                && (IndicatedEndTimeInRemoveingTour <= 
                clsData.getPOI()[TourItinerary[RightVisitIndex].POI_ID - 2][8 + clsData.getNumberOfPersons()])) 
        {
            ...
            if (IndicatedTourSpentTimeInRemoveingTour <= clsData.getStartPoint()[6]) {
                ...
                else
                {
                    if(SubTourReversed){
                        TimeWindowFeasibilityOfPrecedingPOIsInRemoveingTour=
                                this.getTimeWindowFeasibilityOfAffectedPOIsInReversingPartOfTour(
                                LeftVisitIndex, RightVisitIndex, SubTourItinerary, 
                                 IndicatedEndTimeInRemoveingTour, DistanceFromInsertedPointToNextPointInRemoveingTour);
                    }
                    else
                    {
                        TimeWindowFeasibilityOfPrecedingPOIsInRemoveingTour=
                    ...
                    if(TimeWindowFeasibilityOfPrecedingPOIsInRemoveingTour){
                        ...
                        float IndicatedEndTimeInInsertingTour = IndicatedStartTimeInInsertingTour + 
                                clsData.getPOI()[TourItinerary[LeftVisitIndex].POI_ID - 2][3];
                        if((IndicatedStartTimeInInsertingTour >= 
                            clsData.getPOI()[TourItinerary[LeftVisitIndex].POI_ID - 2]
                                [4 + clsData.getNumberOfPersons()])
                            && (IndicatedEndTimeInInsertingTour <= 
                            clsData.getPOI()[TourItinerary[LeftVisitIndex].POI_ID - 2]
                                [8 + clsData.getNumberOfPersons()]))
                        {
                            float IndicatedTourSpentTimeInInsertingTour=IndicatedTourSpentTimeInRemoveingTour;
                            if(IndicatedTourSpentTimeInInsertingTour <= clsData.getStartPoint()[6]){
    ...
    float getShiftingValue(int LeftPOIID, int RightPOIID, 
    ...
    float getDistanceFromPreviosPoint(int POIVisitIndeks,
        int POI_ID, clsVisit TourItinerary[]){
        float result;
        if (POIVisitIndeks == 0) {
            result =
                    clsGeneral.getDistanceBetweenPoints(1, POI_ID, clsData.getDistance());
        } 
        else {
            result =
    ...
    float getDistanceFromPreviosPointWithPossibleSwapBetweenNeighbourPOIs(int LeftPOIVisitIndex, int RightPOIVisitIndex,
...
//                    POI_ID, clsData.getDistance());    
    ...
    float getDistanceToNextPoint(int POIVisitIndeks,int POI_ID,
        ...
        float result;
            ...
            result =
                    clsGeneral.getDistanceBetweenPoints(POI_ID,
                    TourItinerary[POIVisitIndeks+1].POI_ID, clsData.getDistance());
            }
        return result;
    }
    float getDistanceToNextPointWithPossibleSwapBetweenNeighbourPOIs(int LeftVisitIndex,int RightVisitIndex,int POI_ID,
...
//                TourItinerary[LeftVisitIndex].POI_ID, clsData.getDistance());
                ...
                result =
                clsGeneral.getDistanceBetweenPoints(POI_ID,
                TourItinerary[LeftVisitIndex+1].POI_ID, clsData.getDistance());
            }       
        }
    ...
    boolean getTimeWindowFeasibilityOfAffectedPOIsInReversingPartOfTour(int LeftVisitIndex,int RightVisitIndex, 
        ...
        for(int i=0;i<SubTourItinerary.length;i++){
            if(IndicatedStartTime>=clsData.getPOI()[SubTourItinerary[i].POI_ID - 2][4 + clsData.getNumberOfPersons()]){
                IndicatedEndTime=IndicatedStartTime+ clsData.getPOI()[SubTourItinerary[i].POI_ID - 2][3];
                if(IndicatedEndTime >  clsData.getPOI()[SubTourItinerary[i].POI_ID - 2][8 + clsData.getNumberOfPersons()]){
                    result=false;
                    break;
                }
            }
    ...
    float getInitialSolutionTimeCost(){
        float result;
            result=clsData.getStartPoint()[6];
        return result;
    }
   ...
   clsVisit [] createNewTourItinerary(clsVisit TourItinerary[],int TourLastVisit, int LeftPOIIndex, int RightPOIIndex,
    ...
    float getSubtourLength(clsVisit SubTourItinerary[], int LeftPOI_ID, int RightPOI_ID){
        ...
        float getTwoOptChangeLength(clsVisit SubTourItinerary[], int LeftPOI_ID, int RightPOI_ID){
        float result=clsGeneral.getDistanceBetweenPoints(RightPOI_ID, 
                SubTourItinerary[0].POI_ID, clsData.getDistance())+
                clsGeneral.getDistanceBetweenPoints(SubTourItinerary[SubTourItinerary.length -2].POI_ID, 
                LeftPOI_ID, clsData.getDistance());
        return result;
    }
